...
class Scanner {
  ...
  getScanner(firstLine, position, anchorPosition) {
    ...
    if (firstLine) {
      if (position === anchorPosition) {
        return this.firstLineAnchoredScanner != null
          ? this.firstLineAnchoredScanner
          : (this.firstLineAnchoredScanner = this.createScanner(
              firstLine,
              position,
              anchorPosition,
            ));
      } else {
        return this.firstLineScanner != null
          ? this.firstLineScanner
          : (this.firstLineScanner = this.createScanner(firstLine, position, anchorPosition));
      }
    } else if (position === anchorPosition) {
      return this.anchoredScanner != null
        ? this.anchoredScanner
        : (this.anchoredScanner = this.createScanner(firstLine, position, anchorPosition));
    } else {
  ...
  findNextMatch(line, firstLine, position, anchorPosition) {
    // A scan from position p returns the leftmost match starting at or after
    // p, so its result stays valid for any later position up to that match's
    // start (or for the rest of the line when nothing matched). Anchored
    // scanners are excluded because a literal \G in the pattern resolves to
    // the search-start position, making results position-dependent.
    if (!this.anchored) {
      const cached = this.cachedResult;
      if (cached != null && cached.string === line && cached.position <= position) {
        if (cached.match == null) {
          return null;
        }
        if (position <= cached.match.captureIndices[0].start) {
          return cached.match;
        }
      }
    }
    ...
    if (!this.anchored) {
      this.cachedResult = { string: line, position: position, match: match };
    }
    return match;
  }
