 ...
 *
 * This class should not be instantiated directly. Obtain one from a
 * {@link GrammarRegistry} by calling {@link GrammarRegistry#loadGrammar}.
 */
class Grammar {
  constructor(registry, options) {
    ...
    this.foldingStopMarker = options.foldingStopMarker;
   ...
   * @status public
   *
   * Invoke the given callback when this grammar is updated due to a grammar it
   * depends on being added or removed from the registry.
   *
   * @param {Function} callback - Function to call when this grammar is updated.
   * @returns {Disposable} A subscription on which `.dispose()` can be called to unsubscribe.
   */
   ...
   *
   * @param {String} text - Text containing one or more lines.
   * @param {Boolean} [compatibilityMode=true] - Whether to use compatibility results while tokenizing each line.
   * @returns {Array<Array<Object>>} Token arrays for each tokenized line.
   */
   ...
   * * `tags`: An `Array` containing positive token lengths and negative scope
   *   identifiers. Negative odd values begin scopes and negative even values
   *   end them. Resolve identifiers with `scopeForId` on the
   *   {@link GrammarRegistry}.
   * * `tokens`: A lazily decoded `Array` of token objects with `value` and
   *   `scopes` properties. Reading it incurs additional overhead. This property
   *   is present when compatibility mode is enabled.
   * * `ruleStack`: An `Array` of rules representing the tokenized state at the
   *   end of the line. Pass it back to this method when tokenizing the next
   *   line in the file.
   *
   * @param {String} inputLine - Text to tokenize.
   * @param {Array<Object>|null} [ruleStack=null] - Rules returned by the previous call, or `null` for the first line.
   * @param {Boolean} [firstLine=false] - Whether this is the first line in the file.
   * @param {Boolean} [compatibilityMode=true] - Whether to expose the lazy `tokens` property.
   * @param {Boolean} [appendNewLine=true] - Whether to include a terminating newline while matching.
   * @returns {Object} The tokenization result described above.
   */
  tokenizeLine(inputLine, ruleStack, firstLine, compatibilityMode, appendNewLine) {
    ...
    // Only the scanned string needs to live in WASM memory; `line` is a plain
    // JS string because patterns only read it via substring/length. The
    // OnigString must be disposed before returning or the WASM heap leaks.
    let string = line;
    let stringWithNewLine = new oniguruma.OnigString(appendNewLine ? line + "\n" : line);
    if (ruleStack != null) {
    ...
    while (true) {
      ...
      let match = _.last(ruleStack).rule.getNextTags(
        ruleStack,
        string,
        stringWithNewLine,
        position,
      ...
      if (position === previousPosition) {
        if (ruleStack.length === previousRuleStackLength) {
          console.error(
            `Popping rule because it loops at column ${position} of line ${line}`,
            _.clone(ruleStack),
          );
          ...
          // Stack size increased with zero length match
          ...
          if (
            (lastRule != null ? lastRule.scopeName : void 0) != null &&
            penultimateRule.scopeName === lastRule.scopeName
          ) {
            popStack = true;
          }
  ...
  scopesFromStack(stack, rule, endPatternMatch) {
    ...
    if (
      endPatternMatch &&
      (rule != null ? rule.contentScopeName : void 0) &&
