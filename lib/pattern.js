...
// If a pattern doesn't have a regex, we can run into trouble. Instead of
// enforcing the presence of a regex and possibly breaking grammars in the
// wild, let's fall back to a pattern that will never match anything.
const DEFAULT_REGEX = "\\A(?!x)(?=x)";
...
class Pattern {
  constructor(grammar, registry, options) {
    ...
    if (match) {
      if (
        (end || this.popRule) &&
        (this.hasBackReferences != null
          ? this.hasBackReferences
          : (this.hasBackReferences = DigitRegex.test(match)))
      ) {
        this.match = match;
      } else {
  ...
  getRegex(firstLine, position, anchorPosition) {
    if (this.anchored) {
      ...
      return this.regexSource || DEFAULT_REGEX;
    }
  }
  ...
  resolveScopeName(scopeName, line, captureIndices) {
    return scopeName.replace(
      AllCustomCaptureIndicesRegex,
  ...
  handleMatch(stack, line, captureIndices, _rule, _endPatternMatch) {
    ...
    if (this.popRule) {
      ...
      if (
        zeroWidthMatch &&
        _.last(stack).zeroWidthMatch &&
        _.last(stack).rule.anchorPosition === captureIndices[0].end
      ) {
        return false;
      }
    ...
    if (this.captures) {
      tags.push.apply(
        tags,
        this.tagsForCaptureIndices(line, captureIndices.slice(), captureIndices, stack),
      );
    } else {
  ...
  tagsForCaptureRule(rule, line, captureStart, captureEnd, stack) {
    ...
    let tags = rule.grammar.tokenizeLine(
      captureText,
      __slice.call(stack).concat([
        {
          rule: rule,
        },
      ]),
  ...
  tagsForCaptureIndices(line, currentCaptureIndices, allCaptureIndices, stack) {
    ...
    let tags = [];
    let scope =
      this.captures[parentCapture.index] != null ? this.captures[parentCapture.index].name : void 0;
    ...
    let captureRule =
      this.captures[parentCapture.index] != null ? this.captures[parentCapture.index].rule : void 0;
    if (captureRule) {
      captureTags = this.tagsForCaptureRule(
        captureRule,
        line,
        parentCapture.start,
        parentCapture.end,
      ...
      while (currentCaptureIndices.length && currentCaptureIndices[0].start < parentCapture.end) {
        ...
        captureTags = this.tagsForCaptureIndices(
          line,
          currentCaptureIndices,
          allCaptureIndices,
          stack,
        );
        tags.push.apply(tags, captureTags);
