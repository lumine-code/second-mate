...
class GrammarRegistry {
          ...
          if (error != null) {
            ...
            if (typeof grammar.scopeName === "string" && grammar.scopeName.length > 0) {
              return typeof callback === "function"
                ? callback(null, _this.createGrammar(grammarPath, grammar))
                : void 0;
            } else {
              return typeof callback === "function"
  ...
  grammarUpdated(scopeName) {
    for (let i = 0; i < this.grammars.length; i++) {
      const grammar = this.grammars[i];
      if (grammar.scopeName !== scopeName && grammar.grammarUpdated(scopeName)) {
        if (Grammar.includeDeprecatedAPIs) {
    ...
    for (let i = 0; i < tags.length; ++i) {
      ...
      if (tag >= 0) {
        ...
        if (poppedScopeName !== expectedScopeName) {
          throw new Error(
            `Expected popped scope to be ${expectedScopeName}, but it was ${poppedScopeName}`,
          );
        }
      }
    }
