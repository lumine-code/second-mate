...
class Rule {
  ...
  normalizeCaptureIndices(line, captureIndices) {
    ...
    for (let i = 0; i < captureIndices.length; i++) {
      ...
      capture.index = i
      // This is kinda weird - the Atom-era Oniguruma binding, when `length === 0`,
      // sets `start` and `end` to the same as the old capture's `start` and
      // `end`. This _probably is not_ a feature, and more like a leftover on
      // how indexes used to be handled, but for some reason, some grammars
      // like XML and Shellscript work *better* with this format (beating
      // vscode's grammars). So we patch results here to have the same behavior
      if(capture.length === 0) {
        if(capture.start === 0xffffffff) {
          capture.end = 0
          capture.start = 0
        } else if(i > 0) {
          const oldCapture = captureIndices[i-1]
          capture.start = oldCapture.start
          capture.end = oldCapture.start
        }
      }
  ...
  findNextMatch(ruleStack, lineWithNewline, position, firstLine) {
    ...
    const nextMatch = scanner.findNextMatch(lineWithNewline, firstLine, position, this.anchorPosition);
    if (nextMatch) {
