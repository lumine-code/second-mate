...
const __waitFor = chaiWaitFor.bindWaitFor({
  timeout: 1000,
  retryInterval: 50,
});
  ...
  describe("maxLineLength option", () => {
    it("limits the number of characters scanned by the parser per line", () => {
      ...
      const { line, tags, ruleStack } = grammar.tokenizeLine(
        '{"foo": "this is a long value"}',
        initialRuleStack,
      );
      const tokens = registry.decodeTokens(line, tags);

      expect(ruleStack.map((entry) => entry.scopeName)).to.eql(
        initialRuleStack.map((entry) => entry.scopeName),
      );
