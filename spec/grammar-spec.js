  ...
  describe("::tokenizeLine(line, ruleStack)", () => {
    ...
    describe("when the line matches no patterns", () => {
      it("does not infinitely loop", () => {
        ...
        expect(registry.decodeTokens(line, tags)).to.eql([
          { value: "hoo", scopes: ["text.plain", "meta.paragraph.text"] },
        ]);
      });
    });
    ...
    describe("when the line matches a pattern with no `name` or `contentName`", () => {
      it("creates tokens without adding a new scope", () => {
        ...
        expect(tokens[1]).to.eql({
          value: "oh ",
          scopes: ["source.ruby", "string.quoted.other.literal.lower.ruby"],
        });
        expect(tokens[2]).to.eql({
          value: "\\l",
          scopes: ["source.ruby", "string.quoted.other.literal.lower.ruby"],
        });
        expect(tokens[3]).to.eql({
          value: "ook",
          scopes: ["source.ruby", "string.quoted.other.literal.lower.ruby"],
        });
      });
    ...
    describe("when the line matches a begin/end pattern", () => {
      it("returns tokens based on the beginCaptures, endCaptures and the child scope", () => {
        ...
        expect(tokens[1]).to.eql({
          value: "single-quoted heredoc",
          scopes: ["source.coffee", "string.quoted.heredoc.coffee"],
        });
        expect(tokens[2]).to.eql({
          value: "'''",
          scopes: [
      ...
      describe("when the pattern spans multiple lines", () => {
        it("uses the ruleStack returned by the first line to parse the second line", () => {
          ...
          ({ line: line2, tags: tags2 } = grammar.tokenizeLine("heredoc'''", ruleStack));
          ...
          expect(firstTokens[1]).to.eql({
            value: "single-quoted",
            scopes: ["source.coffee", "string.quoted.heredoc.coffee"],
          });

          expect(secondTokens[0]).to.eql({
            value: "heredoc",
            scopes: ["source.coffee", "string.quoted.heredoc.coffee"],
          });
          expect(secondTokens[1]).to.eql({
            value: "'''",
            scopes: [
              "source.coffee",
              "string.quoted.heredoc.coffee",
      ...
      describe("when the pattern contains sub-patterns", () => {
        it("returns tokens within the begin/end scope based on the sub-patterns", () => {
          ...
          expect(tokens[1]).to.eql({
            value: "heredoc with character escape ",
...
`);
          ...
          expect(lines[1][2]).to.eql({
            value: "}excentricSyntax",
            scopes: ["source.apply-end-pattern-last", "end-pattern-last-env", "scope", "excentric"],
          });
          expect(lines[4][2]).to.eql({
            value: "}",
            scopes: ["source.apply-end-pattern-last", "normal-env", "scope"],
          });
          expect(lines[4][3]).to.eql({
            value: "excentricSyntax }",
            scopes: ["source.apply-end-pattern-last", "normal-env"],
          });
        });
      });
      ...
      describe("when the end pattern contains a back reference", () => {
        ...
        it("allows the rule containing that end pattern to be pushed to the stack multiple times", () => {
          grammar = registry.grammarForScopeName("source.ruby");
          const { line, tags } = grammar.tokenizeLine(
            "%Q+matz had some #{%Q-crazy ideas-} for ruby syntax+ # damn.",
          );
          const tokens = registry.decodeTokens(line, tags);
          ...
          expect(tokens[1]).to.eql({
            value: "matz had some ",
            scopes: ["source.ruby", "string.quoted.other.literal.upper.ruby"],
          });
          expect(tokens[2]).to.eql({
            value: "#{",
            scopes: [
          ...
          expect(tokens[7]).to.eql({
            value: " for ruby syntax",
            scopes: ["source.ruby", "string.quoted.other.literal.upper.ruby"],
          });
          expect(tokens[8]).to.eql({
            value: "+",
            scopes: [
              "source.ruby",
              "string.quoted.other.literal.upper.ruby",
      ...
      describe("when the pattern includes rules from another grammar", () => {
        describe("when a grammar matching the desired scope is available", () => {
          it("parses tokens inside the begin/end patterns based on the included grammar's rules", () => {
            ...
            const { line, tags } = grammar.tokenizeLine(
              "<div class='name'><%= User.find(2).full_name %></div>",
            );
            const tokens = registry.decodeTokens(line, tags);
            ...
            expect(tokens[2]).to.eql({
              value: " ",
              scopes: ["text.html.ruby", "meta.tag.block.any.html"],
            });
            expect(tokens[3]).to.eql({
              value: "class",
              scopes: [
            ...
            expect(tokens[6]).to.eql({
              value: "name",
              scopes: ["text.html.ruby", "meta.tag.block.any.html", "string.quoted.single.html"],
            });
            expect(tokens[7]).to.eql({
              value: "'",
              scopes: [
            ...
            expect(tokens[10]).to.eql({
              value: " ",
              scopes: ["text.html.ruby", "source.ruby.rails.embedded.html"],
            });
            expect(tokens[11]).to.eql({
              value: "User",
              scopes: ["text.html.ruby", "source.ruby.rails.embedded.html", "support.class.ruby"],
            });
            expect(tokens[12]).to.eql({
              value: ".",
              scopes: [
                "text.html.ruby",
                "source.ruby.rails.embedded.html",
            ...
            expect(tokens[13]).to.eql({
              value: "find",
              scopes: ["text.html.ruby", "source.ruby.rails.embedded.html"],
            });
            expect(tokens[14]).to.eql({
              value: "(",
              scopes: [
                "text.html.ruby",
                "source.ruby.rails.embedded.html",
            ...
            expect(tokens[18]).to.eql({
              value: "full_name ",
              scopes: ["text.html.ruby", "source.ruby.rails.embedded.html"],
            });
            expect(tokens[19]).to.eql({
              value: "%>",
              scopes: [
                "text.html.ruby",
                "source.ruby.rails.embedded.html",
          ...
          it("updates the grammar if the included grammar is updated later", async () => {
            ...
            let { line, tags } = grammar.tokenizeLine(
              "<div class='name'><% <<-SQL select * from users;",
            );
            let tokens = registry.decodeTokens(line, tags);
          ...
          it("supports including repository rules from the other grammar", () => {
            ...
            expect(tokens[0]).to.eql({
              value: "enumerate",
              scopes: ["test.include-external-repository-rule", "support.function.builtin.python"],
            });
    ...
    it("can parse a grammar that captures the same text multiple times (regression)", () => {
      ...
      expect(lines[0][1]).to.eql({
        value: "b",
        scopes: ["abcabx", "abx", "up-to-x-outer", "up-to-x-inner"],
      });
    ...
    describe("when a grammar has a pattern that has back references in the match value", () => {
      it("does not special handle the back references and instead allows oniguruma to resolve them", () => {
        ...
        expect(tokens[9]).to.eql({
          value: "-moz-selector",
          scopes: ["source.css.scss", "meta.property-list.scss", "meta.property-name.scss"],
        });
      });
    });
    ...
    describe("when a line has more tokens than `maxTokensPerLine`", () => {
      it("creates a final token with the remaining text and resets the ruleStack to match the begining of the line", () => {
        ...
        const { line, tags, ruleStack } = grammar.tokenizeLine(
          "var x = /[a-z]/;",
          originalRuleStack,
        );
        const scopes = [];
    ...
    describe("when the grammar has injections", () => {
      it("correctly includes the injected patterns when tokenizing", () => {
        ...
        expect(tokens[14].scopes).to.eql([
          "text.html.php",
          "meta.embedded.line.php",
        ...
        expect(tokens[15].scopes).to.eql([
          "text.html.php",
          "meta.embedded.line.php",
          "punctuation.section.embedded.end.php",
        ]);
        ...
        expect(tokens[16].scopes).to.eql([
          "text.html.php",
          "meta.tag.block.any.html",
          "punctuation.definition.tag.begin.html",
        ]);
        ...
        expect(tokens[17].scopes).to.eql([
          "text.html.php",
          "meta.tag.block.any.html",
          "entity.name.tag.block.any.html",
        ]);
      ...
      it("updates injections when grammars that the injection patterns use are updated", () => {
        ...
        expect(tokens[1]).to.eql({
          ...
          scopes: ["source.sql-injection", "string", "meta.embedded.sql"],
        });
        ...
        expect(tokens[1]).to.eql({
          value: "SELECT",
          scopes: ["source.sql-injection", "string", "meta.embedded.sql", "keyword.other.DML.sql"],
        });
      });
      ...
      it("gives lower priority to them than other matches", () => {
        ...
        const { line, tags } = grammar.tokenizeLine(
          "<script><?php function hello() {} ?></script>",
        );
        const tokens = registry.decodeTokens(line, tags);
    ...
    describe("when the grammar has an injection selector", () => {
      ...
      it("gives lower priority to them than other matches", () => {
        ...
        expect(tokens[0].value).to.eql("<!--");
        expect(tokens[0].scopes).not.to.eql([
          "source.js",
          "should-not-be-matched.normal.injection-selector",
        ]);
        expect(tokens[0].scopes).to.eql([
          "source.js",
    ...
    describe("when the grammar has a prefixed injection selector", () => {
      it("correctly prioritizes them when tokenizing", () => {
        ...
        expect(tokens[0].scopes).to.eql([
          "source.js",
          "should-be-matched.prefixed.injection-selector",
        ]);
      });
    });
    ...
    describe("Unicode support", () => {
      ...
      describe("when the line contains emoji characters", () => {
        it("correctly terminates quotes & parses tokens starting after them", () => {
          ...
          expect(withoutEmojiTokens[7].scopes).to.eql([
            "source.js",
          ...
          expect(withEmojiTokens[7].scopes).to.eql([
            "source.js",
            "punctuation.terminator.statement.js",
          ]);
        });
